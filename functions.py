...
def shuffle_deal3(All_Players, Trumper, Deck, MA_cards):
	Rest_Deck = list(Deck)
	shuffle(Rest_Deck)
	for i in range(1,4):
		MA_cards[Rest_Deck[0]]['Owner'] = Trumper
		Trumper.hand[Trumper.place+str(i)] = Rest_Deck.pop(0)
	return Rest_Deck
...
def deal_rest_cards(All_Players, Trumper, Rest_Cards, MA_cards):
	for player in All_Players:
		if player.name == Trumper.name:
			n = All_Players.index(player) + 1
...
def reset_streech(All_Players, MA_cards):
	for player in All_Players:
		...
		for card in player.hand:
			MA_cards[player.hand[card]]['Owner'] = ""
